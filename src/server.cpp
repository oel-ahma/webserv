...
Server::Server(ConfigParse const &conf, Config const &allconf) : ready(false), config(&conf), allConfig(&allconf) {
}
...
bool Server::send(int socket) {
	if (this->response_buff[socket].responseIsSet == false && !this->client_buff[socket].getRequest().empty()) {
		...
		if (!ifStringInVector(client_buff[socket].getHeaders()["Host"], this->config->getServerName()))
		{
			...
			if (this->response_buff[socket].responseIsSet == false)
				 response_buff[socket].prepareResponse(&client_buff[socket], &allConfig->_servers[0]);
		}
		else
			response_buff[socket].prepareResponse(&client_buff[socket], this->config);
	}
